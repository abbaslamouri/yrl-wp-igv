...
const renderChart = function (chart, sheet, containers) {
  ...
  function drawDashboard() {
    ...
    if (
      ...
      chart.chartParams.enableTableChart
    ) {
      // Create a dashboard.
      ...
      console.log("RANMGE", chart.chartParams.enableChartRangeSlider);
      ...
      if (
        ...
        document
          .getElementById("iwpgv__num-range-filter")
      ...
      if (
        ...
        // Unhide number range container div
        document
          .getElementById("iwpgv__chart-range-filter")
          .classList.remove("hidden");
      ...
      if (chart.chartParams.enableTableChart) {
        ...
        document
          .getElementById("iwpgv__table-chart")
          .classList.remove("hidden");
      }
    ...
    if (chart.chartParams.enableMinMaxTableChart) {
      ...
      // Unhide number range container div
      document
        .getElementById("iwpgv__min-max-table-chart")
        .classList.remove("hidden");
    }

    // Hide number rabge slider if not enabled
    if (!chart.chartParams.enableNumRangeSlider) {
    ...
    if (!chart.chartParams.enableTableChart) {
      ...
      // Unhide table cahrt container div
      document.getElementById("iwpgv__table-chart").classList.add("hidden");
    }
...
const chartOptionsChange = function (chart, sheet, containers) {
  ...
  if (!event.target.classList.contains("iwpgv__chart")) return;
  ...
  if (fieldId === "iwpgv__chartOptions[width]") {
    const chartOptionsWidth = document.getElementById(
      "iwpgv__chartOptions[width]"
    );
    fieldVal = setChartWidth(chartOptionsWidth, "iwpgv__dashboard");
  }

  if (
    fieldId === "iwpgv__chartOptions[chartArea][width]" ||
    fieldId === "iwpgv__chartOptions[chartArea][height]"
  ) {
    fieldVal = resizeChartArea(fieldId);
  }

  // Set range chart width/height and chart area width/height
  if (fieldId === "iwpgv__chartRangeOptions[ui][chartOptions][width]") {
    const chartRangeOptionsWidth = document.getElementById(
      "iwpgv__chartRangeOptions[ui][chartOptions][width]"
    );
    fieldVal = setChartWidth(chartRangeOptionsWidth, "iwpgv__filter-min-max");
  }
  if (
    fieldId ===
      "iwpgv__chartRangeOptions[ui][chartOptions][chartArea][width]" ||
    fieldId === "iwpgv__chartRangeOptions[ui][chartOptions][chartArea][height]"
  ) {
    fieldVal = resizeChartArea(fieldId);
  }
...
const resizeChart = function (chart, sheet, containers) {
  ...
  document.getElementById("iwpgv__chart").style.opacity = 0;
  iwpgvChart.setOption("width", "1");
  iwpgvChart.draw();
  iwpgvChart.setOption("width", setChartWidth(chart, "iwpgv__dashboard"));
  iwpgvChart.draw();
  ...
  timeout = setTimeout(function () {
    ...
    document.getElementById("iwpgv__chart").style.opacity = 1;
  }, 2000);
};

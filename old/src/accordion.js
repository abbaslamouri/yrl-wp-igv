...
Note:  the div with .accordion__content must directly follow the div with .accodion__content
*/
...
class Accordion {
  constructor(options) {
    ...
    const accordions = document.querySelectorAll(".iwpgv .accordion");
    ...
    accordions.forEach((accordion) => {
      ...
      for (const prop in children) {
        if (
          children[prop].classList &&
          children[prop].classList.contains("accordion__toggle")
        )
          toggles[prop] = children[prop];

        if (
          children[prop].classList &&
          children[prop].classList.contains("accordion__content")
        )
          contents[prop] = children[prop];
      }
    ...
    // this.openAccordion(accordions[0].querySelector(".accordion__content"));
    // accordions[
  }

  toggleAccordion(content, useElem) {
    if (getComputedStyle(content).height == "0px") {
      useElem.href.baseVal = `${iwpgv_obj.url}assets/img/icons.svg#icon-keyboard_arrow_down`;
      this.openAccordion(content);
    } else {

...
// }
		...
		public function dashboard_submenu_page_render() {
			...
			if (isset($_GET["action"])) {
				...
				$chart_id = (isset($_GET["chart-id"]))? $_GET["chart-id"] :  null;
				$chart = (isset($_GET["chart-id"]))? get_option("{$this->prefix}_dashboard")[$_GET["chart-id"]] : null;
				...
				if ($chart_id) {
					...
					// check if a file is found
					if (! $file) {
						$errors->add("no_file_found", __("File id ({$chart_id}) submitted but no file found in the database", $this->plugin));
					} else {

						// Fetch sheet data 
						$sheet = $this->fetch_spreadsheet_data($file->file_path)["sheet-data"][$chart["sheet-select"]]["formatted"];
						$sheet_name = $this->fetch_spreadsheet_data($file->file_path)["sheet-data"][$chart["sheet-select"]]["sheet-name"];
				...
				add_meta_box(
					($chart_id)? "{$this->plugin}-edit-chart-{$chart_id}-metabox" : "{$this->plugin}-add-new-chart-metabox",
					($chart_id)? "Edit Chart {$chart_id}" : "Add New Chart",
					function ($object, $params) { // object is passed as first parameter from do_meta_boxes, $params[$args] is the last argument in add_meta_box
						
						echo $this->chart_metabox_render("edit-chart");
				...
				add_meta_box(
					...
					'side',
					'default',
					array("chart-id" => $chart_id, "chart" => $chart)
				);
				...
				// If there are charts 
					...
					foreach ($charts as $chart_key => $chart) {
						...
						// $chart_id = $chart(isset($_GET["chart-id"]))? $_GET["chart-id"] :  null;
						// $chart = (isset($_GET["chart-id"]))? get_option("{$this->prefix}_dashboard")[$_GET["chart-id"]] : null;
						...
						if (! $file) {
							...
							// Fetch sheet data 
							$sheets[$chart_key] = array(
								"sheet-data" => $this->fetch_spreadsheet_data($file->file_path)["sheet-data"][$chart["sheet-select"]]["formatted"],
								"sheet_name" => $this->fetch_spreadsheet_data($file->file_path)["sheet-data"][$chart["sheet-select"]]["sheet-name"],
								"file-name"  => $file->name
							);
						...
						add_meta_box(
							...
							function ($object, $params) {  // object is passed as first parameter from do_meta_boxes, $params[$args] is the last argument in add_meta_box
								...
								<!-- <form id ="chart-delete-form" action = "">
									<input type ="hidden" id = "chart-id" name="chart-id" value = "<?php //echo $chart_key ?>" >
									<?php //submit_button("Delete Chart", 'secondary', "chart-delete-submit", false);?>
								</form>

								<a href ="<?php //echo add_query_arg(array("page" => $this->plugin, "action" => "edit-chart", "chart-id" => $chart_key), admin_url('admin.php')); ?>" class ="add-new-h2" id ="add-new-chart">Edit Chart</a> -->
								...
								echo $this->chart_metabox_render($chart_key); 
							},
							$this->dashboard_hook,
							'normal',
							'default',
							array('chart_key' => $chart_key)
						);				
		...
		*/
		...
		public function chart_metabox_render($div_key = null) {

			// Strat buffer
			ob_start();

			?>
			<div id = "<?php echo "dashboard-{$div_key}-div"; ?>" ></div>
			<div id = "<?php echo "filter-{$div_key}-div"; ?>" ></div>
			<div id = "<?php echo "chart-{$div_key}-div"; ?>" ></div>
			<?php
				...
				foreach($this->admin_sections() as $section) {
					...
					<div class = "panel">
						...
						foreach ($this->admin_fields() as $field) {
							...
							if (isset($chart[$field["id"]])) {
								$field["value"] = $chart[$field["id"]];
							}

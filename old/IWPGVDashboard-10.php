...
if (!class_exists('IWPGVDashboard')) { 
	class IWPGVDashboard {
		...
		public $default_file_name = "annotations.xlsx";
		...
		*/
			...
			if (isset($_GET["action"])) {
				...
				if ($_GET["action"] == "addNewChart") {
					...
					if (! is_array($spreadsheet)) {
						...
						$chart_options = $options->chart_options($field_defaults);

						// Add series to admin field panels
						$series = $this->fetch_series ($sheet, "series");
						$admin_field_panels["series"] = $series["series"];

						//echo "<pre>"; var_dump($series);die;

						// Fetch series defaults (return fields only.  This is used to set default chart options)
						$series_defaults = array();
						foreach( $this->fetch_panel_fields($series) as $field) {
							$series_defaults[$field["id"]] = $field["default"];
						}

						// Add series options to chart options
						$chart_options["series"] = $this->fetch_series_options($series_defaults);


						// Add series to admin field panels
						$trendlines = $this->fetch_series ($sheet, "trendlines");
						$admin_field_panels["trendlines"] = $trendlines["series"];

						//echo "<pre>"; var_dump($trendlines_series);die;
						...
						// Add series options to chart options
						$chart_options["trendlines"] = $this->fetch_series_options($trendlines_defaults);
		...
		public function fetch_relevant_field_panels ($admin_field_panels, $sheet){
			...
			// Unset annotations subpanel if annotation flag is flase
			if (! $annotation_flag) {

				// Loop through all admin fields, find and unset annotations subpanel
				foreach ($admin_field_panels as $panel_id => $panel) {
					foreach($panel["subpanels"] as $subpanel_id => $subpanel) {
						if (false !== strpos($subpanel_id, "annotations"))
			...
			foreach ($admin_field_panels as $panel_id => $panel) {
				//echo $panel_id; echo "<br>";var_dump($panel);echo "<br><br><br>";
				foreach($panel["subpanels"] as $subpanel_id => $subpanel) {
					if ( isset($subpanel["chartTypes"]) && ! in_array($this->default_chart_type, $subpanel["chartTypes"])) 
						unset($admin_field_panels[$panel_id]["subpanels"][$subpanel_id]);
				}
			}

jQuery(document).ready(function($) {
	...
	var x = document.getElementsByClassName("chartOption");
	console.log(x);
  ...
  if ($("#iwpgv-dashboard select[id='chart.fileId']").find("option").length > 1) {
   	$("#iwpgv-dashboard select[id='chart.fileId']").parent(".fieldWrapper").slideDown();
  } else {
	...
	// Hode sheet select field
	$("#iwpgv-dashboard select[id='chart.chartType']").parent(".fieldWrapper").slideUp();


	//******************************************  Upload new file   *******************************

	 $(document).on("click", "input, select", function(event) {

		event.stopPropagation(); // Stop stuff happening
		event.preventDefault(); // Totally stop stuff happening

	 	if (event.target.id == "fileUploadSubmit") {

			$("#iwpgv-dashboard select[id='chart.fileId']").val("");
			$("#iwpgv-dashboard select[id='chart.sheetId']").val("").parent(".fieldWrapper").slideUp();
	...
	$(document).on("change", "input, select", function(event) {
		...
		if (event.target.id == "chart.fileId" || event.target.id == "chart.sheetId" || event.target.id == "chart.chartType" ) {
			...
			if ($(this).attr("id") == "chart.fileId") {
				...
				// hise sheetId and chart type selection field
			   	sheetSelect.val(null).parent(".fieldWrapper").slideUp();
			   	chartTypeSelect.val(null).parent(".fieldWrapper").slideUp();
	...
	function refresh(formData){
	  ...
	  if ( ! fileSelect.val() ) {
	  		...
	  		// Apppend file id to sorm
	  		formData.append( fileSelect.attr("id").split(".")[1], fileSelect.val());
	  ...
	  if ( ! sheetSelect.val() ) {
			...
			message= "<div class='notice notice-error is-dismissible'>Please select a sheet</div>" ;
	  		...
	  		// Apppend sheet id to sorm
	  		formData.append( sheetSelect.attr("id").split(".")[1], sheetSelect.val());
	  ...
	  if ( ! chartTypeSelect.val()) {
			...
			message= "<div class='notice notice-error is-dismissible'>Please select a chart type</div>" ;
	  		...
	  		// Apppend chart type id to sorm
	  		formData.append( chartTypeSelect.attr("id").split(".")[1], chartTypeSelect.val());
	...
	// 	// Display Messages
	// 	$("#iwpgv-dashboard #adminMessages").append(response.message);
	// 	$("#iwpgv-dashboard #empty-chart").html("");

	// 	// Render chart and admin fileds
	// 	renderAll(response);
		...
		$.ajax ({
			...
			success: function(response, status, jqXHR) {
			  response = JSON.parse(response);
				...
				if (response.post.action === iwpgv_object.refresh_chart_action) renderAll(response);
	...
	function renderAll(response) {
		...
		if (response) {
			...
			$("#iwpgv-dashboard #dashboard-admin-fields").html(response.relevantFieldPanels);

			$("#iwpgv-dashboard #dashboard-admin-fields [id='chart.fileId']").val(response.post.fileId);
			//$("#iwpgv-dashboard #dashboard-admin-fields [id='chart.sheetId']").val(response.post.sheetId);
			$("#iwpgv-dashboard #dashboard-admin-fields [id='chart.chartType']").val(response.post.chartType);
			...
			chartType = response.post.chartType;
			...
			// Add % suffix to annotations gradient 
			chartOptions.annotations.boxStyle.gradient.x1 = chartOptions.annotations.boxStyle.gradient.x1 + "%";
			chartOptions.annotations.boxStyle.gradient.y1 = chartOptions.annotations.boxStyle.gradient.y1 + "%";
			chartOptions.annotations.boxStyle.gradient.x2 = chartOptions.annotations.boxStyle.gradient.x2 + "%";
			chartOptions.annotations.boxStyle.gradient.y2 = chartOptions.annotations.boxStyle.gradient.y2 + "%";
			...
			var allInputs = $("#iwpgv-dashboard #iwpgv-admin-fields select, #iwpgv-dashboard #iwpgv-admin-fields input").not("#iwpgv-dashboard #iwpgv-admin-fields #fileUploadSubmit");
	...
	function getDatatable(sheet) {
		...
		$.each (sheet.labels, function (i, label) {
			data.addColumn({
				...
				label: label,
				role: ( sheet.roles )? sheet.roles[i] : null
				});
		});
...
(function ( $ ) {
	 ...
	 $.fn.iwpgvAccordion = function() {
		...
		this.on("click", "button.accordionButton", function(event) {

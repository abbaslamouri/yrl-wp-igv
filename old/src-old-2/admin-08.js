jQuery(document).ready(function($) {
			...
			if (!! inputClass && inputClass.includes("chartOption")) {
			 ...
			 // if input has class numRangeOption 
				...
				inputId = (!! parts[2])? parts[1] + "." + parts[2] : parts[1];
		 		...
		 		var numRangeFilterInputs = $("#iwpgv-dashboard .panel#numRangeFilter input, #iwpgv-dashboard .panel#numRangeFilter select");
				...
				if ( inputId == "enable") {
					...
					$(input).change(function() {
		 				if ($(this).prop("checked")){
		 				 	filterColumnLabel.parent(".fieldWrapper").slideDown();
		 				 	...
		 				 	$("#iwpgv-dashboard select[id='numRangeFilter.filterColumnLabel']").val(null);
		 				 	$.each (numRangeFilterInputs, function (i, numRangeFilterinput) {
								if (numRangeFilterinput.id != "numRangeFilter.enable")
									$(numRangeFilterinput).parent(".fieldWrapper").slideUp();
							});
				...
				if ( inputId == "filterColumnLabel") {
					...
					$(input).change(function() {
						...
						if ($(this).val()) {
							...
							var numRangeSliderOptions = response.numRangeSliderOptions;
							numRangeSliderOptions.filterColumnLabel = $(this).val();

						 	//Render chart
							renderChart(response.sheet, response.divId, response.chartType, response.chartOptions, response.numRangeSliderOptions );
	...
	function renderChart(sheet, divId, chartType, chartOptions, numRangeFilterOptions= null) {

jQuery(document).ready(function($) {
   ...
   $("#" + templateId + " .hint").mouseout(function(){
	...
	function fileUploadRender(data) {
		...
		if (! data.message.includes("notice-error")) {
			...
			fileId.append(data.fileIdOption).val(data.fileId).parent(".fieldWrapper").slideDown();
	...
	$(document).on("change", "#" + templateId + " select", function(event) {
		...
		if ($(this).attr("id") == "fileId") {
		   ...
		   if (! $(this).val()) {
		   	...
		   	$(sheetIdSelect).parent(".fieldWrapper").slideUp();
	...
	function refreshChart(response) {
		...
		renderChart(response.sheet, response.divId, response.post.chartType, response.chartOptions);
							...
							$("#" + templateId + " #dashboard-admin-fields").find("#sheetId").val(response.post.sheetId).parent(".fieldWrapper").removeClass("hideOnLoad");
  ...
  // Loop through all the showTrendlines checkboxes and hide corresponsding input fields and removove series trendlieines if the checkbox is unchecked
	...
	function renderChart(sheet, divId, chartType, chartOptions) {
		...
		// var newTrendlines = {};
		// $.each(chartOptions.trendlines, function(i, option){
		// 	if (! option.type) newTrendlines[i] = {}; 
		// });
		...
		function drawDashboard() {
			...
			//if (chartType == "LineChart") filterColumnLabel = sheetId.labels.B;

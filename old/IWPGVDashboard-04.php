...
if (!class_exists('IWPGVDashboard')) { 
	class IWPGVDashboard {
		...
		public $data_types = array("string", "number", "boolean", "datetime", "timeofday"); // Possible cahrt types
		...
		public function file_upload_callback() {
			...
			try{
				...
				foreach ($spreadsheet as $key => $sheet) {
					...
					foreach($sheet["data"]["cols"] as $col_key => $value ) {
						if (isset($value["type"]) && ! in_array($value["type"], $this->data_types)) {
							$invalid_data_type = true;
							$col_data_type = $value["type"];
							$sheet_name = $sheet["sheetName"];
							$sheet_column = "{$value["label"]} ({$value["id"]})";
							break 2;
						}
					}
				}
				...
				if (isset($invalid_data_type)) {
					$error = "File: <strong>". sanitize_file_name($file["name"]) . "</strong> Sheet <strong>{$sheet_name}</strong> column <strong>{$sheet_column}</strong> <strong>{$col_data_type}</strong> is not a valid data type.  Allowed file extensions are  <strong>".implode(", ", $this->data_types)."</strong>This file was not uploaded.";
				...
				// If no files exists, initialise file id to strat at 1235 (file id will be incremented by 1 for each new file)
				if (empty($files)) {
				...
				if (isset($files[$file_id])) {
					wp_delete_file(wp_upload_dir()['path']."/".sanitize_file_name($file["name"]));
					throw new \Exception ( __("This file ({$file["name"]}) sould not be in option <strong>{$this->prefix}_dashboard", $this->plugin));
				}
		...
		*/
		public function file_select_callback() {
			...
			try {
				...
				// Retreive files settings options
				$file = ($settings && isset($settings["files"]))? $settings["files"][$_POST["fileId"]] : null;
				...
				$sheet_select_options = $this->compose_sheet_select_options($spreadsheet, $_POST["fileId"]);
		...
		*/
		public function chart_save_callback() {
			...
			try {
				...
				// If no charts exists, initialise chart id to strat at 1235 (file id will be incremented by 1 for each new file)
				if (empty($charts)) {
					$chart_id = $this->min_chart_id;
				
				// If there are file in the database, find max file id and increment by 1
				} else {
					$max_chart_id = $this->min_file_id;
				...
				foreach ($this->admin_fields() as $field) {
					if (isset($_POST[$field["id"]])) $chart_parts[$field["id"]] = $_POST[$field["id"]];
				}
				...
				// Add success message
				$message = "<div class='notice notice-success is-dismissible'><p>Chart saved successfully</p></div>";
		...
		public function compose_sheet_select_options ($spreadsheet, $selected = null)  {
			...
			foreach ($spreadsheet as $sheet_key => $sheet) {
				$sheet_select_options .= "<option value='".(intval($sheet_key))."'>{$sheet["sheetName"]}</option>";
			}
...
// }
		...
		public function dashboard_submenu_page_render() {
			...
			if (isset($_GET["action"])) {
				...
				// Get chart
				$chart = (isset($_GET["chartId"]))? get_option("{$this->prefix}_dashboard")[$_GET["chartId"]] :array();

				// Loop through all admin fields to get field values
				$field_values = array();
				if (! empty($chart)) {
					foreach ($this->admin_fields() as $field) {
						$field_values[$field["id"]]= $chart[$field["id"]];
					}
				}
				...
				if ($_GET["action"] == "addNewChart") {
					...
					// If a file id was submitted
					if ($chart_id) {
						
						// Fetch file
						$file = $db->find_by_id($chart_id);

						// check if a file is found
						if (! $file) {
							$this->errors->add("no_file_found", __("File id ({$chart_id}) submitted but no file found in the database", $this->plugin));
						} else {

							$spreadsheet = $this->fetch_spreadsheet($file->file_path);

							//$sheet_name = $spreadsheet["sheet-names"];
							// Fetch sheet data 
							$sheet = array(
								"sheet-data" => $spreadsheet["sheet-data"][$chart["sheetId"]]["formatted"],
								"sheet_name" => $spreadsheet["sheet-data"][$chart["sheetId"]]["sheet-name"],
								"file-name"  => $file->name
							);

							// check if a sheet is found in the spreadsheet
							if (! $sheet) {
								$this->errors->add("no_sheet_found", __("Sheet id ({$chart["sheetId"]}) submitted but no sheet found in the spreadsheet", $this->plugin));
							} 
				...
				add_meta_box(
					"{$this->plugin}-{$chart_id}-metabox",
					"{$file_name} - {$sheet["sheetName"]}",
					function ($object, $params) { // object is passed as first parameter from do_meta_boxes, $params[$args] is the last argument in add_meta_box
						$chart_id = (isset($params["args"]["chartId"]))? $params["args"]["chartId"] : "editChart";
						$action = $params["args"]["action"];

						echo $this->chart_metabox_render("editChart", $action);
					},
					$this->dashboard_hook,
					'normal',
					'default',
				...
				add_meta_box(
					...
					function ($object, $params) { // object is passed as first parameter from do_meta_boxes, $params[$args] is the last argument in add_meta_box
						...
						echo $this->admin_fields_metaboxes_render($chart_id, $chart, $sheet);
			...
			// If not new chart or edit chart request is submitted
				...
				// If there are charts 
					...
					foreach ($charts as $chart_id => $chart) {
						...
						// Fetch file
						$file = $files[$chart["fileSelect"]]; //s = $settings["files"];

						// check if a file is found
						if (! $file) {
							$this->errors->add("no_file_found", __("File id ({$chart["fileSelect"]}) submitted but no file found in the database", $this->plugin));
						} else {
						...
						add_meta_box(
							...
							"File Name: {$output[$chart_id]["fileName"]},   Sheet Name: {$sheet["sheetName"]}",
							function ($object, $params) {  // object is passed as first parameter from do_meta_boxes, $params[$args] is the last argument in add_meta_box
								$chart_id = $params["args"]["chartId"];
								//$chart = $params["args"]["chart"];
								?>
		...
		*/
		...
		public function chart_metabox_render($divId, $action= null) {
			...
			if ($action) submit_button("Refresh", 'primary', "refresh", false); 
		 ...
		 */
		function admin_fields_metaboxes_render($chart_id, $chart, $sheet) {

			// Initialize file select dield options
			$file_select_options = [];

			?><div id ="dashbboard-admin-fields"><?php

				// Loop through admin sections
				foreach($this->admin_sections() as $section) {
					?>
					<button class ="accordion"><?php echo $section["title"] ?></button>
					<div class = "panel" id="<?php echo $section["id"] ?>">
						<?php
						...
						foreach ($this->admin_fields() as $field) {
							...
							// Populte each secdtion with the appropriate fields
...
return $output;
		...
		*/
		...
		public function default_chart_options($options) {
			
			return array(
				"chart" => array(
					"title" => $options["chartTitle"],
				),
				"titlePosition" => $options["chartTitlePosition"],
				...
				public function admin_fields() {
					...
					return array(
						...
						array(
							...
							"fieldTitle" => __("Select Sheet", $this->plugin), // Formatted title of the field. Shown as the label for the field during output.
							"submenuPage" => "{$this->prefix}_dashboard", // Section to which this field belongs
							"section" => "fileSettings",
							"fieldType" => "select", // custom field (fieled\\d type:  text, email, checkbox, textarea....) (supplied in the $args)
							"fieldOptions" => array(),
						...
						array(
							...
							"fieldTitle" => __("Select Chart Type", $this->plugin), // Formatted title of the field. Shown as the label for the field during output.
							"submenuPage" => "{$this->prefix}_dashboard", // Section to which this field belongs
							"section" => "fileSettings",
							"fieldType" => "select", // custom field (fieled\\d type:  text, email, checkbox, textarea....) (supplied in the $args)
						...
						array(
							...
							"id" => "chartTitle",  // id attribute if the field
							"fieldTitle" => __("Chart Title", $this->plugin), // Formatted title of the field. Shown as the label for the field during output.
							"submenuPage" => "{$this->prefix}_dashboard", // Section to which this field belongs
						...
						array(
							//"slug" => chart_title, // field id
							"id" => "chartTitlePosition",  // id attribute if the field
							"fieldTitle" => __("Chart Title Position", $this->plugin), // Formatted title of the field. Shown as the label for the field during output.
							"submenuPage" => "{$this->prefix}_dashboard", // Section to which this field belongs
							"section" => "chartSettings",
							"fieldType" => "select", // custom field (fieled\\d type:  text, email, checkbox, textarea....) (supplied in the $args)
							"fieldOptions" => array(
								"in" => "in - Draw the title inside the chart area",
								"out" => "out - Draw the title outside the chart area",
								"none" => "none - Omit the title"
							),
							"nullOption" => "Select Chart Title Position",
							//description => Enable Custom Post Types, // Custom field description(supplied in the $args)
							//value => (isset($settings) && isset($settings[chart_title])) ? $settings[chart_title] : "",
							"default" => "out"
						),
						...
						array(
							//"slug" => chart_title, // field id
							"id" => "chartSubtitle",  // id attribute if the field
							"fieldTitle" => __("Chart Subtitle", $this->plugin), // Formatted title of the field. Shown as the label for the field during output.
							"submenuPage" => "{$this->prefix}_dashboard", // Section to which this field belongs
							"section" => "chartSettings",
							"fieldType" => "text", // custom field (fieled\\d type:  text, email, checkbox, textarea....) (supplied in the $args)
							//description => Enable Custom Post Types, // Custom field description(supplied in the $args)
						...
						array(
							...
							"id" => "chartTitleColor",  // id attribute if the field
							"cssClass" => "color-picker",
							"fieldTitle" => __("Chart Title Color", $this->plugin), // Formatted title of the field. Shown as the label for the field during output.
							"submenuPage" => "{$this->prefix}_dashboard", // Section to which this field belongs
							"section" => "chartSettings",
						...
						array(
							//"slug" => chart_title, // field id
							"id" => "chartTitleFontName",  // id attribute if the field
							"fieldTitle" => __("Chart Title Font", $this->plugin), // Formatted title of the field. Shown as the label for the field during output.
							"submenuPage" => "{$this->prefix}_dashboard", // Section to which this field belongs
							"section" => "chartSettings",
							"fieldType" => "select", // custom field (fieled\\d type:  text, email, checkbox, textarea....) (supplied in the $args)
							"fieldOptions" => array(
						...
						array(
							//"slug" => chart_title, // field id
							"id" => "chartTitleBold",  // id attribute if the field
						...
						array(
							//"slug" => chart_title, // field id
							"id" => "chartTitleItalic",  // id attribute if the field

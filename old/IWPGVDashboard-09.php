...
if (!class_exists('IWPGVDashboard')) { 
	class IWPGVDashboard {
		...
		*/
		public function refresh_chart_callback() {
			...
			try {
				...
				// Fetch trendline field options
							$trendlines = $this->fetch_trendlines($sheet);

							// Fetch trendlines subpanel to be rendered in the options
							$trendline_fields = $this->fetch_trendlines_subpanel ($trendlines);
		...
		*/
			...
			if (isset($_GET["action"])) {
				...
				if ($_GET["action"] == "addNewChart") {
					...
					if ($result !== true) {
						...
						$sheet = $this->fetch_spreadsheet($file_path);
						...
						if (! is_array($sheet)) {
							...
							foreach( $this->fetch_all_fields($this->default_chart_type) as $field) {
								$field_defaults[$field["id"]] = $field["default"];
							}

							// Set chart options
							$chart_options = $options->default_chart_options($field_defaults, $this->default_chart_type);

							// Retreive series and fetch series field defaults
							$series = $this->fetch_series ($sheet, "series");

							$series_defaults = array();
							foreach( $series["subpanels"] as $subpanel) {
							...
							foreach($series_defaults as $field_id => $field_value) {
							...
							$chart_options["series"] = array_shift($series_options);
							...
							$trendline_defaults = array();
							foreach( $trendlines["subpanels"] as $subpanel) {
								foreach($this->fetch_subpanel_fields($subpanel) as $field) {
									$trendline_defaults[$field["id"]] = $field["default"];
								}
							}

							// compose series field options
							$trendline_options = array();
							foreach($trendline_defaults as $field_id => $field_value) {
								...
								switch (count($parts)) {
									...
									case 6:
										$trendline_options[$parts[0]][$parts[1]][$parts[2]][$parts[3]][$parts[4]][$parts[5]] = $field_value;
										break;

									case 5:
										$trendline_options[$parts[0]][$parts[1]][$parts[2]][$parts[3]][$parts[4]] = $field_value;
										break;

									case 4:
										$trendline_options[$parts[0]][$parts[1]][$parts[2]][$parts[3]] = $field_value;
										break;

									case 3:
										$trendline_options[$parts[0]][$parts[1]][$parts[2]] = $field_value;
										break;

									case 2:
										$trendline_options[$parts[0]][$parts[1]] = $field_value;
										break;

									case 1:
										$trendline_options[$parts[0]] = $field_value;
										break;
							...
							// Add series options to chart options
							$chart_options["trendlines"] = array_shift($trendline_options);


								// Compose series subpanel fields;
							ob_start();?>
							<button class='accordionButton series'>Series</button>
							<div class = 'panel' id='series'>
								<?php $this->fetch_subpanels ($series["subpanels"]); ?>
							</div>
							<!-- <button class='accordionButton trendlines'>Trendlines</button> -->
							<!-- <div class = 'panel' id='trendlines'> -->
								 <?php //$this->fetch_subpanels ($trendlines["subpanels"]); ?>
							<!-- </div> --><?php
							$extra_panels = ob_get_clean();
							ob_end_flush();

							$this->add_chart_metabox ($file_name, $sheet, $divId, $_GET["action"] );

							$this->add_admin_fields_metabox();
				...
				$response = array(
					"divId"		=> "editChart",
					"sheet"			=> $sheet,
					"fileName" 	=> (isset($file_name) && $file_name)? $file_name : "",
		...
		public function fetch_all_fields($chart_type) {
			...
			foreach ($options->option_fields($chart_type) as  $panel_id => $panel) {
			...
			//echo "<pre>";var_dump($all_fields);die;
				...
				if  ( isset($field["id"])) {
					...
					foreach($field as $subfield) {
						if  ( isset($subfield["id"])) {
							array_push($subpanel_fields, $subfield);
						} else {
							foreach($subfield as $subsubfield) {
								array_push($subpanel_fields, $subsubfield);
							}
						}
					}
				}
		...
		*/
		...
		public function add_admin_fields_metabox($chart = null, $sheet = null) {
			add_meta_box(
			...
			<div id ="dashboard-admin-fields" class = "accordion">
				...
				<?php foreach($options->option_fields($this->default_chart_type, null) as $option_id => $option) { ?>
					...
					<div class = "panel" id = "<?php echo $option_id ?>" >
						<div class="panelIntro"><?php echo (isset($option["intro"]))? $option["intro"] : ""; ?></div>
						<?php $this->fetch_subpanels ($option["subpanels"]); ?>
					</div><!-- .panel -->

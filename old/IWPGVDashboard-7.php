			...
			foreach ($sheet_names as $sheet_key => $sheet_name) {
				...
				foreach ($data as $row_index => $row) {
					...
					foreach ($row as $col_index => $cell ) {
						if (isset($data[1][$col_index]) && $data[1][$col_index] && isset($data[2][$col_index]) && $data[2][$col_index] && isset($cell) && $cell)
			...
			if (isset($_GET["action"])) {
				...
				if ($_GET["action"] == "addNewChart") {
					...
					// If no errors 
						...
						// Set default file name if no chart is being edited
						$file = $file_instance->file_parts("{$this->path}assets/{$this->default_file_name}");
						...
						// If no eirrors 
							...
							$field_values = array();
							foreach ($this->admin_settings($this->default_chart_type, null) as $panel_id => $panel) {
							...
							//$chart_options["trendlines"] = $trendline_parts;
										...
										//if ($series_options_key == 0) continue;
									 	if (count($trendlines["subpanels"]) != 1) :?>
											<button class ="accordionButton" ><?php echo $panel["title"] ?></button>
												<div class = "panel <?php echo $panel["id"]  ?>" id="<?php echo "series-{$i}" ?>">
													Show Trendlines <input type="checkbox" id ="<?php echo $i ?>" class="showTrendlines">
													<div class = "trendlineFields">
										<?php  endif;
		...
		*/
		...
		function admin_fields_metaboxes_render($chart, $sheet) {
			...
			<div id ="dashbboard-admin-fields" class = " accordion">
				...
				<?php foreach($this->admin_settings($this->default_chart_type, null) as $setting_id => $setting) { ?>
				
					<button  class ="accordionButton"><?php echo $setting["title"] ?></button>
					<div class = "panel" id = "<?php echo $setting_id ?>" >
						<div class = "subPanel accordion" >
							<!-- Loop through admmin fields -->

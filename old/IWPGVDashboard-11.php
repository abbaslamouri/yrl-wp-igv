...
if (!class_exists('IWPGVDashboard')) { 
	class IWPGVDashboard {
		...
		*/
		...
		public function file_upload_callback() {
			...
			try{
				...
				$spreadsheet = $this->fetch_spreadsheet($file_path);
				...
				foreach ($spreadsheet as $key => $sheet) {
					...
					// loop through the sheet roles types
		...
		*/
		public function refresh_chart_callback() {
			...
			try {
				...
				// get field defaults (this is used to set default chart options)
				$field_defaults = array();
				foreach( $this->fetch_panel_fields($_POST["chartType"]) as $field) {
				...
				// Retreive trendline defaults
				$trendlines = $this->fetch_series ($sheet, "trendlines");

				// Retreive all admin fields
				$admin_fields = $options->admin_field_panels($_POST["chartType"]);
					
				// Add series to admin fields
				$admin_fields["series"] = $series;
				
				// Add trendlines to admin fields
				$admin_fields["trendlines"] = $trendlines;


				//filter panels
				$new_admin_fields = array();
				foreach ($admin_fields as $panel_id => $panel) {
					if ( in_array($_POST["chartType"], $panel["chartTypes"])) {
						$new_admin_fields[$panel_id] = $panel;
						foreach($new_admin_fields[$panel_id] as $subpanel_id => $subpanel) {
							if ( in_array($_POST["chartType"], $subpanel["chartTypes"])) {
								$new_admin_fields[$panel_id][$subpanel_id] = $subpanel;
							} else {
								continue;
							}
						}
						...
						continue;
								...
								//echo json_encode($admin_fields);die;
				...
				// 	} else {
				// 		if (($field["dataType"] === "continuous" && $dataType = "number") || ($field["dataType"] === "discrete" && $dataType = "string") )
				// 			array_push($subpanel_fields, $field);
				// 	}
							
				// Compose admin fields render
				$admin_fields_render = $this->render_admin_field_panels($new_admin_fields);

				// Compose sheet selection options
				$sheet_id_options = $this->compose_sheet_id_options($spreadsheet, $_POST["fileId"]);

				// Set chart options
				$chart_options = $options->chart_options($field_defaults); //ns to chart options
				$chart_options["series"] = $this->fetch_series_options($series["subpanels"]);

				// Add trendline optiuons to chart options
				$chart_options["trendlines"] = $this->fetch_series_options($trendlines["subpanels"]);
			...
			$response = array(
				...
				"adminFields" => $admin_fields_render,
				"post" => array("action" => $_POST["action"], "chartType" => $_POST["chartType"], "sheetId" => $_POST["sheetId"]),
			);
		...
		*/
		 ...
		 * @return Array         Array contains a file selection from (String) and a sheet selection form (String)
		 */
		public function compose_filter_columns_options ($sheet, $filter, $selected = null)  {

			$output = array( "numberColumn" => null, "stringColumn" => null );
			...
			if ( "number_range_filter" == $filter  || "chart_range_filter" == $filter ) {


				foreach($sheet["roles"] as $key => $value) {
					if ($value == "domain" && isset($sheet["labels"][$key]) && $sheet["labels"][$key]) $filter_columns[$i] = $sheet["labels"][$key];
					$i++;
				}

			} elseif ( "category_filter" == $filter || "string_filter" == $filter ) {

				foreach($sheet["dataTypes"] as $key => $value) {
					if ($value == "string" && isset($sheet["labels"][$key]) && $sheet["labels"][$key]) $filter_columns[$i] = $sheet["labels"][$key];
					$i++;
				}

			}

			//echo "<pre>"; var_dump($filter_columns);

			$filter_columns_options = "";
			foreach ($filter_columns as $filter_columns_key => $filter_column) {
				$filter_columns_options .= "<option value='{$filter_columns_key}' ".selected(intval($filter_columns_key), $selected, false).">{$filter_column}</option>";
			}
			
			return $filter_columns_options;

						$num_range_filter_columns_options = $this->compose_filter_columns_options ($sheet, "number_range_filter", null);

							// Compose doamin options
						$string_filter_columns_options = $this->compose_filter_columns_options ($sheet, "string_filter", null);

						// Compose doamin options
						$category_filter_columns_options = $this->compose_filter_columns_options ($sheet, "category_filter", null);

						// Compose doamin options
						$chart_range_filter_columns_options = $this->compose_filter_columns_options ($sheet, "C", null);
		...
		*/
		...
		public function dashboard_submenu_page_render() {
			...
			if (isset($_GET["action"])) {
				...
				if ($_GET["action"] == "addNewChart") {
					...
					// If all is good an array is returned by fetch spreadsheet
					if (! is_array($spreadsheet)) {
						$errors = __($spreadsheet, $this->plugin);

						// Render chart metabox
						$this->add_chart_metabox ($file_name, array(), $divId, $_GET["action"] );
					} else {
						...
						$admin_field_panels = $this->insert_array($admin_field_panels, $series, "series", $insertion_index+1); //echo "<pre>"; var_dump($admin_field_panels);die;
						...
						//echo "<pre>"; var_dump($series_defaults);die;
						...
						// Compose doamin options
						$chart_range_filter_columns_options = $this->compose_filter_columns_options ($sheet, "chart_range_filter", null);
						...
						$relevant_field_panels = $this->fetch_relevant_field_panels ($admin_field_panels, $sheet);
											...
											//echo "<pre>"; var_dump($filter_columns);die;


					

						//echo "<pre>"; var_dump($domain_options);die;

						//echo "<pre>"; var_dump($relevant_field_panels);die;

						// Compose admin fields render
						$this->add_admin_fields_metabox($relevant_field_panels);

						// Render chart metabox
						$this->add_chart_metabox ($file_name, $sheet, $divId, $_GET["action"] );
				...
				$response = array(
					...
					"numRangeFilterColumnOptions" => $num_range_filter_columns_options,
					"categoryFilterColumnOptions" => $category_filter_columns_options,
					"stringFilterColumnOptions" => $string_filter_columns_options,
					"chartRangeFilterColumnOptions" => $chart_range_filter_columns_options,
		...
		public function fetch_relevant_field_panels ($admin_field_panels, $sheet){
			...
			foreach ($admin_field_panels as $panel_id => $panel) {
				...
				if ( isset($panel["chartTypes"]) && ! in_array($this->default_chart_type, $panel["chartTypes"])) {
		...
		public function fetch_series ($sheet, $panel){
			...
			foreach($sheet["labels"] as $label_key => $label) {
				if ($sheet["roles"][$label_key] != "data") continue;
		...
		public function add_chart_metabox ($file_name, $sheet, $divId, $action ) {
			...
			add_meta_box(
				...
				(isset($sheet) && ! empty($sheet))? "{$file_name} - {$sheet["sheetName"]}" : "{$file_name}" ,
		...
		function admin_fields_metaboxes_render($object, $params) {
			...
			<div id ="dashboard-admin-fields" class = "accordion">
				<?php $this->render_panels($panels); ?>
					<button class="button button-primary", id="saveChart">Save Chart</button>
			</div> <!-- accordion -->
			<?php

		...
		public function file_select_callback() {
			...
			// If file exists
  			if ($file) {
				...
				// Add success message
				$success->add("file_selection_successful", __("File selection successful", $this->plugin));
			...
			if (! ($_POST["file-id"])) {
				...
				// If file exists
  				if ($file) {
					...
					// Fetch spreadsheet data 
					$sheet = $this->fetch_sheet_data(wp_upload_dir()['path']."/".sanitize_file_name($file->name), $_POST["sheet-id"]);

					// Add success message
					$success->add("sheet_selection_successful", __("sheet selection successful", $this->plugin));
		...
		*/
		public function chart_save_callback() {
			...
			if ( 'POST' !== $_SERVER['REQUEST_METHOD'] || ! defined( 'DOING_AJAX' ) || ! DOING_AJAX || ! isset($_POST["action"]) || $_POST["action"] != "{$this->prefix}_chart_save_hook") 
				$errors->add("invalid_chart_save_ajax_request", __("This is not a valid ajax chart save request", $this->plugin));
			
				// verify file upload nonce
			if (! wp_verify_nonce($_POST["nonce"], "{$this->prefix}__{$this->chart_save_nonce}"  )) 
				$errors->add("invalid_{$this->chart_save_nonce}", __("An invalid nonce was submited", $this->plugin));
			...
			if (! $_POST["file-select"] ) {
				...
				if ( $_POST["sheet-select"] == "") {
					...
					$chart_key =  "{$_POST["file-select"]}-{$_POST["sheet-select"]}";
...
// }
		...
		public function dashboard_submenu_page_render() {
			...
			if (isset($_GET["action"])) {
				...
				//$_GET["action"] == "add-new-chart") || $_GET["action"] == "edit-chart")) {

				$chart_key = (isset($_GET["chart-key"]))? $_GET["chart-key"] :  null;
				$chart = (isset($_GET["chart-key"]))? get_option("{$this->prefix}_dashboard")[$_GET["chart-key"]] : null;
				...
				if ($chart_key) {
					...
					// Fetch file
					$file = $db->find_by_id($chart_key);

					// check if a file is found
					if (! $file) {
						$errors->add("no_file_found", __("File id ({$chart_key}) submitted but no file found in the database", $this->plugin));
					} else {

						$spreadsheet = $this->fetch_spreadsheet_data($file->file_path);

						//$sheet_name = $spreadsheet["sheet-names"];
						// Fetch sheet data 
						$sheet = array(
							"sheet-data" => $spreadsheet["sheet-data"][$chart["sheet-select"]]["formatted"],
							"sheet_name" => $spreadsheet["sheet-data"][$chart["sheet-select"]]["sheet-name"],
							"file-name"  => $file->name
						);

						// check if a sheet is found in the spreadsheet
						if (! $sheet) {
							$errors->add("no_sheet_found", __("Sheet id ({$chart["sheet-select"]}) submitted but no sheet found in the spreadsheet", $this->plugin));
						} 
				...
				add_meta_box(
					($chart_key)? "{$this->plugin}-edit-chart-{$chart_key}-metabox" : "{$this->plugin}-add-new-chart-metabox",
					($chart_key)? "Edit Chart {$chart_key}" : "Add New Chart",
					function ($object, $params) { // object is passed as first parameter from do_meta_boxes, $params[$args] is the last argument in add_meta_box
						//$chart_key = (isset($params["args"]["chart-key"]))? $params["args"]["chart-key"] : "edit-chart";
						echo $this->chart_metabox_render($params);

					},
					$this->dashboard_hook,
					'normal',
					'default',
					array("chart-key" => "edit-chart")
				);
				...
				add_meta_box(
					...
					function ($object, $params) { // object is passed as first parameter from do_meta_boxes, $params[$args] is the last argument in add_meta_box
						...
						echo $this->admin_fields_metaboxes_render($params);
					
					},
					$this->dashboard_hook,
					'side',
					'default',
					array("chart-key" => $chart_key, "chart" => $chart, "spreadsheet" => $spreadsheet)
				);
				...
				// If there are charts 
					...
					foreach ($charts as $chart_key => $chart) {
						...
						// $chart_key = $chart(isset($_GET["chart-key"]))? $_GET["chart-key"] :  null;
						// $chart = (isset($_GET["chart-key"]))? get_option("{$this->prefix}_dashboard")[$_GET["chart-key"]] : null;
						...
						// check if a file is found
						if (! $file) {
							$errors->add("no_file_found", __("File id ({$chart["file-select"]}) submitted but no file found in the database", $this->plugin));
						} else {
							...
							if (empty($spreadsheet)) {
								$errors->add("no_spreadsheet_data_found_{$chart_key}", __("Sheet id ({$chart["sheet-select"]}) submitted but no data found in the spreadsheet.", $this->plugin));
							} else {
								
								// Check if sheet has data 
								$sheet_data = $spreadsheet["sheet-data"][$chart["sheet-select"]]["formatted"];

								if (! $sheet_data) {
									$errors->add("no_sheet_found_{$chart_key}", __("Sheet id ({$chart["sheet-select"]}) submitted but no sheet found in the spreadsheet", $this->plugin));
								} else {

									// Prepare output
									$output[$chart_key] = array(
										"chart-key"		=> $chart_key,
										"sheet-data"	=> $sheet_data,
										"sheet-name"	=> $spreadsheet["sheet-data"][$chart["sheet-select"]]["sheet-name"],
										"file-name" 	=> $file_instance->file_parts($file->file_path)["basename"],
										"chart"			=> $chart,
									);
								}
							}
						...
						add_meta_box(
							...
							"File Name: {$output[$chart_key]["file-name"]},   Sheet Name: {$output[$chart_key]["sheet-name"]}",
							function ($object, $params) {  // object is passed as first parameter from do_meta_boxes, $params[$args] is the last argument in add_meta_box
								$chart_key = $params["args"]["chart-key"];
								//$chart = $params["args"]["chart"];
								?>
								<!-- <form id ="chart-delete-form" action = "">
									<input type ="hidden" id = "chart-key" name="chart-key" value = "<?php //echo $chart_key ?>" >
									<?php //submit_button("Delete Chart", 'secondary', "chart-delete-submit", false);?>
								</form> -->

								<a href ="<?php echo add_query_arg(array("page" => $this->plugin, "action" => "edit-chart", "chart-key" => $chart_key), admin_url('admin.php')); ?>" class ="add-new-h2" id ="add-new-chart">Edit Chart</a>
		...
		*/
		...
		public function chart_metabox_render($params) {
			...
			<div id = "<?php echo "chart-{$chart_key}-div"; ?>" ></div>
			<?php
		 ...
		 */
		function admin_fields_metaboxes_render($params) {
		...
		// Instantiate the database class
			...
			// Retreive chart parameters
			$chart_key = $params['args']['chart-key'];
			$chart = $params['args']['chart'];
			$spreadsheet = $params['args']['spreadsheet'];
			...
			foreach($this->admin_sections() as $section) {
				...
				<div class = "panel">
					...
					foreach ($this->admin_fields() as $field) {
						...
						if ($field["slug"] == "sheet_select") $field["field_options"] = $spreadsheet["sheet-names"];
						if ($field["slug"] == "file_select") $field["field_options"] = $file_select_options;
		 ...
		 */
		function add_edit_chart_metabox_render($params) {
		...
		*/
		...
		public function sheet_selection_render($params)
		{
			...
			if (count($sheets)> 1 ) {
				...
				<input type = "hidden" id ="chart-key" name = "chart-key" value = "">
...
return $html;
				...
				*/
				...
				$chart_key =  "{$input["file-select"]}-{$input["sheet-select"]}";

				//echo $chart_key;

				$charts[$chart_key] = $input;

...
class Accordion {
  ...
  toggleAccordion(content, useElem) {
    if (getComputedStyle(content).height == "0px") {
      useElem.href.baseVal = `${iwpgvObj.url}assets/img/icons.svg#icon-keyboard_arrow_down`;
      this.openAccordion(content);
    } else {
      useElem.href.baseVal = `${iwpgvObj.url}assets/img/icons.svg#icon-keyboard_arrow_right`;
      this.closeAccordion(content);
    }
  }

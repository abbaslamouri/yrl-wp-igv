...
const drawChart = async function (target, iwpgvObj, prefix, spreadsheet) {
  ...
  try {
    ...
    if ( !form ) throw new Error(  "<div class='notice notice-error is-dismissible'><p>Something went terribly wrong, we cannot find the params form</p></div>" )
  ...
  const fileUpload = document.getElementById(`${prefix}__chartParams[fileUpload]`).value
  const sheetId = document.getElementById(`${prefix}__chartParams[sheetId]`).value
  const chartType = document.getElementById(`${prefix}__chartParams[chartType]`).value
  if ( ! fileUpload || ! sheetId || ! chartType ) return
    ...
    for (const property of formData ) {
      ...
      console.log("ZZZZZZ",inputParams)
      console.log("YYYYYY",fieldId)
    ...
    const chartTraceInstance = new ChartTrace( chartTrace, $index, $labels, $chart_type, prefix )

    // const index = 1;
    // while (index < count($sheet['labels'])) {
      
    //   $trace_option = ( ! empty($traces_options) && isset($traces_options[index]) ) ? $traces_options[index] : [];
    //   $trace_options_instance = new TraceOption( $trace_option , index, $sheet['labels'], $chart_type );
    //   $chart['traceOptions'][index-1] = $trace_options_instance->options();
    //   $chart['traceOptions'][index-1]["x"] = $sheet["data"][0];
    //   $chart['traceOptions'][index-1]["y"] = $sheet["data"][index];
    //   $panels["traceOptions"]['sections'][index-1]['id'] = "{$this->prefix}__traceOptionsPanel__trace[{index}-1]";
    //   $panels["traceOptions"]['sections'][index-1]['title'] = count($sheet['labels']) > 1 ? __(array_values($sheet['labels'])[index], $this->plugin) : "";
    //   $panels["traceOptions"]['sections'][index-1]['fields'] = $trace_options_instance->fields();
    //   $i++;
    // }
    ...
    if (jsonRes.status && jsonRes.status === "success") {
     ...
     // Enable save button
      ...
      Plotly.newPlot(`${prefix}__plotlyChart`, jsonRes.chart.traceOptions, jsonRes.chart.chartLayout, getConfig())

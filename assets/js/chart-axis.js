...
import { chartOptionKey, createPanel, createPanelSections, displayAdminMessage, setSelectFielddOptions, fetchAxisOptions } from "./utilities"
...
const chartAxis =  async( chart, axisType, prefix ) => {
  ...
  try {
    ...
    for (let i = 0; i < axes.length; i++) {
      lastIndex = parseInt(axes[i].split(axisType)[1]) > lastIndex ? parseInt(axes[i].split(axisType)[1]) : lastIndex
    }
    ...
    for (const prop in chart.traces) {
      setSelectFielddOptions ( document.getElementById ( `${prefix}__traces[${prop}][xaxis]` ), fetchAxisOptions ( chart.layout, "xaxis", capitalize ) )
      setSelectFielddOptions ( document.getElementById (`${prefix}__traces[${prop}][yaxis]` ), fetchAxisOptions (chart.layout, "yaxis", capitalize) )
    }

...
if (  yrl_wp_plotly_charts_obj ) {
  ...
  try {
    ...
    mediaUploader.on("select", async function () {
      ...
      const returnedObj = await fileSelect( chart, charts, spreadsheet, wpRestUrl, wpRestNonce, mediaUploader, mainAccordion, prefix )
    ...
    event.preventDefault( )
      ...
      if( event.target.id === `${prefix}__params[sheetId]` ) {
        ...
        chartUpdated = await sheetHandler( chart, event.target.value, spreadsheet, mainAccordion, prefix )
